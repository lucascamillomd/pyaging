...
Exactly one may be set. The column is dropped before the gene pipeline, so it is never
mistaken for a gene. With no such column the cohort is taken to be mouse and a warning
says so; setting two, or letting one vary between samples, is an error.

**Reference group.** Add a boolean ``adata.obs["tage_reference_group"]``; the true rows
are the samples to centre against. Without the column the cohort centres on every
sample, which is the reference pipeline's own default. A column that selects nothing is
