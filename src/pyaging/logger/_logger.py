...
class Logger:
    ...
    def info_insert_adata(
        ...
        **kwargs,
        ...
        message = f"<insert> {key} to {adata_attr} in AnnData Object."
        if log_level in (logging.NOTSET, logging.DEBUG):
            self.debug(message, *args, indent_level=indent_level, **kwargs)
        elif log_level == logging.INFO:
            self.info(message, *args, indent_level=indent_level, **kwargs)
        elif log_level == logging.WARN:
            self.warning(message, *args, indent_level=indent_level, **kwargs)
        elif log_level == logging.ERROR:
            self.error(message, *args, indent_level=indent_level, **kwargs)
        elif log_level == logging.CRITICAL:
            self.critical(message, *args, indent_level=indent_level, **kwargs)
        else:
    ...
    def report_progress(self, percent=None, count=None, total=None, progress_name="", indent_level=1):
        ...
        message = "\r" + format_logging_message(
            f"{progress_name}in progress: {percent:.4f}%",
    ...
    def finish_progress(self, progress_name="", time_unit="s", indent_level=1):
        ...
        if time_unit == "s":
            self.info(
                token + f" {progress_name} [{self.time_passed:.4f}s]",
                indent_level=indent_level,
            )
        elif time_unit == "ms":
            self.info(
                token + f" {progress_name} [{self.time_passed * 1e3:.4f}ms]",
                indent_level=indent_level,
            )
        else:
            raise NotImplementedError
...
def main_tqdm(generator, desc="", indent_level=1, logger=None):
    ...
    """
    if logger is None:
        logger = LoggerManager.main_logger
...
def main_info_insert_adata(key, adata_attr="obsm", indent_level=1, *args, **kwargs):
    LoggerManager.main_logger.info_insert_adata(key, *args, adata_attr=adata_attr, indent_level=indent_level, **kwargs)
